...
*/
	                ...
	                {						
						if (!inner_cids.ContainsKey(portName)) 
						{
							...
							{  
                                ...
                                inner_cid = createInnerComponent (slice_kind,
                                                                  cid, 
																  ic, 
																  curr_inner_cid, 
																  bsPort, 
			...
			{	
				...
				foreach (InnerComponent ic in icList) 
				{
						...
						new Thread ((ParameterizedThreadStart)delegate(object facet_info_obj) 
					{
						...
						AbstractComponentFunctor acf = BackEnd.acfdao.retrieve (ic.Id_abstract_inner);
                        int kind = Constants.kindMapping[acf.Kind];
						...
						inner_cid = createInnerComponent (kind, 
                                                          cid, 
														  ic, 
														  curr_inner_cid, 
														  bsPort, 
				...
				foreach (Slice slice in slice_list)
				{
					...
					if (unit_mapping_dict.TryGetValue (i_host.Id_interface_super_top, out info_list)) 
					{
						foreach (Tuple<int,int[],int> info in info_list) 
						{
							...
							Interface i = BackEnd.idao.retrieveSuper (ic.Id_abstract_inner, slice.Id_interface_slice);
                            		Console.WriteLine ("build_unit_mapping_of_inner: " + ic.Id_abstract_inner + "," + slice.Id_interface_slice + ", i_host.Facet = " + i_host.Facet + ", enclosing_facet = " + enclosing_facet_instance + ", i.Facet = " + i.Facet);
					...
					else 
					{
						foreach (string k in unit_mapping_dict.Keys)
							Console.WriteLine("Wrong configuration ! {0} -- {1}", k, i_host.Id_interface_super_top);
			...
			public static ManagerComponentID createInnerComponent (int kind,
                                                                   ManagerComponentID cid, 
				                                                   InnerComponent ic, 
