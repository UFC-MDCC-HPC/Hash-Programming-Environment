...
public class Interface {
            ...
            if (id_interface_super_top == null) {
              ...
              //  AbstractComponentFunctorApplicationDAO acfadao = new AbstractComponentFunctorApplicationDAO();
                ...
                if (id_interface_super.Equals("")) {
                    ...
                    AbstractComponentFunctor acf = br.ufc.hpe.backend.DGAC.BackEnd.acfdao.retrieve(id_abstract);
                    if (acf.Id_functor_app_supertype != 0)
                    {
                        AbstractComponentFunctorApplication acfaSuper = br.ufc.hpe.backend.DGAC.BackEnd.acfadao.retrieve(acf.Id_functor_app_supertype);
                        Interface iSuper = br.ufc.hpe.backend.DGAC.BackEnd.idao.retrieve(acfaSuper.Id_abstract, id_interface_super_);
                ...
                br.ufc.hpe.backend.DGAC.BackEnd.idao.setInterfaceSuperTop(id_abstract, id_interface, id_interface_super_top);
    ...
    public IList<string> fetchReferences(IDictionary <string, AbstractComponentFunctorApplication> pars) {
      ...
      //  AbstractComponentFunctorDAO acfdao = new AbstractComponentFunctorDAO();
        ...
        AbstractComponentFunctor cThis = br.ufc.hpe.backend.DGAC.BackEnd.acfdao.retrieve(this.Id_abstract);
        AbstractComponentFunctorApplication cSuperApp = br.ufc.hpe.backend.DGAC.BackEnd.acfadao.retrieve(cThis.Id_functor_app_supertype);
        if (cSuperApp != null)
        {
            AbstractComponentFunctor acfsuper = br.ufc.hpe.backend.DGAC.BackEnd.acfdao.retrieve(cSuperApp.Id_abstract);
            IDictionary<string, AbstractComponentFunctorApplication> parsSuper = null;
            collectParameters(pars, cSuperApp, out parsSuper);
            Interface iSuper = br.ufc.hpe.backend.DGAC.BackEnd.idao.retrieve(cSuperApp.Id_abstract, this.Id_interface_super);
        ...
        // Traverse slices.
        IList<Slice> slices = br.ufc.hpe.backend.DGAC.BackEnd.sdao.listByInterface(Id_abstract, Id_interface);

        foreach (Slice s in slices)
        {
            InnerComponent ic = br.ufc.hpe.backend.DGAC.BackEnd.icdao.retrieve(Id_abstract, s.Id_inner); // findInnerComponentOwnerOfSlice(Id_abstract, s.Id_inner); 
            if (ic != null)
            {
                ...
                if (!ic.Parameter_top.Equals(""))
                {
                    ...
                    acfa = acfa == null ? br.ufc.hpe.backend.DGAC.BackEnd.acfadao.retrieve(ic.Id_functor_app) : acfa;                    
                }
                else
                {
                    acfa = br.ufc.hpe.backend.DGAC.BackEnd.acfadao.retrieve(ic.Id_functor_app);
                }

                collectParameters(pars, acfa, out parsSlice);

                Interface i = br.ufc.hpe.backend.DGAC.BackEnd.idao.retrieveByMatching(acfa.Id_abstract, ic.Id_abstract_inner, s.Id_interface_slice);
                AbstractComponentFunctor acfSlice = br.ufc.hpe.backend.DGAC.BackEnd.acfdao.retrieve(acfa.Id_abstract);
    ...
    private InnerComponent findInnerComponentOwnerOfSlice(int Id_abstract, string id_inner)
    {
     //   InnerComponentDAO icdao = new InnerComponentDAO();
      ...
      //  AbstractComponentFunctorApplicationDAO acfadao = new AbstractComponentFunctorApplicationDAO();
        ...
        {
            ic = br.ufc.hpe.backend.DGAC.BackEnd.icdao.retrieve(Id_abstract_traverse, id_inner);
            if (ic == null)
            {
                AbstractComponentFunctor cTraverse = br.ufc.hpe.backend.DGAC.BackEnd.acfdao.retrieve(Id_abstract_traverse);
                AbstractComponentFunctorApplication cSuperApp = br.ufc.hpe.backend.DGAC.BackEnd.acfadao.retrieve(cTraverse.Id_functor_app_supertype);
   ...
   //     SupplyParameterDAO spdao = new SupplyParameterDAO();
     //   AbstractComponentFunctorApplicationDAO acfadao = new AbstractComponentFunctorApplicationDAO();
        ...
        foreach (SupplyParameter sp in spList)
        {
            ...
            if (sp is SupplyParameterComponent)
            {
                ...
                acfaPar = br.ufc.hpe.backend.DGAC.BackEnd.acfadao.retrieve(sp_.Id_functor_app_actual);
                ...
                if (acfaPar == null)
                {
                    ...
                    else // if (sp_.FreeVariable)
                    {
                     ...
                     //   AbstractComponentFunctorParameterDAO acfpdao = new AbstractComponentFunctorParameterDAO();
                        AbstractComponentFunctorParameter acfp = br.ufc.hpe.backend.DGAC.BackEnd.acfpdao.retrieve(sp_.Id_abstract, sp_.Id_parameter);
                        acfaPar = br.ufc.hpe.backend.DGAC.BackEnd.acfadao.retrieve(acfp.Bounds_of);
                        parsSlice.Add(sp_.Id_parameter, acfaPar);
                    }
                }
                else
                           ...
                           IDictionary<string, AbstractComponentFunctorApplication> apars)
    {
        ...
        //SupplyParameterDAO spdao = new SupplyParameterDAO();
        IList<SupplyParameter> sps = br.ufc.hpe.backend.DGAC.BackEnd.spdao.list(acfa.Id_functor_app);
        foreach (SupplyParameter sp in sps)
        {
            if (sp is SupplyParameterComponent)
            {
                SupplyParameterComponent sp_ = (SupplyParameterComponent)sp;
               // AbstractComponentFunctorApplicationDAO acfadao = new AbstractComponentFunctorApplicationDAO();
                AbstractComponentFunctorApplication acfaPar = br.ufc.hpe.backend.DGAC.BackEnd.acfadao.retrieve(sp_.Id_functor_app_actual);
