...
namespace DGAC
{
    ...
    */
    public class BackEnd : IBackEnd
    {
        ...
        public int runApplication(string ID)
        {
        ...
        private void sendRunCommandToWorker(object remoteObject, string file)
        {
            ServerObject worker = (ServerObject)remoteObject;
            worker.runClass(file);
        }
        ...
        /* ranks: ranks das unidades 0,1,...,N-1 do componente
     hash_component_uid na aplicação.
         * resultado esperado: ranks das unidades 0,1,...,N-1 do component
     interno id_inner de hash_component_uid
         */
        public int[] translateRanks(int[] ranks, string hash_component_uid, string id_inner, int id_index_inner)
        {
            ComponentDAO cdao = new ComponentDAO();
            Component c = cdao.retrieve_uid(hash_component_uid);
            int id_concrete = c.Id_concrete;

            UnitSliceDAO sdao = new UnitSliceDAO();

            UnitDAO udao = new UnitDAO();
            IList<database.Unit> us = udao.list(id_concrete);

            IDictionary<int, int> rankMapping = new Dictionary<int, int>();

            int rank_unit = 0;
            foreach (database.Unit u in us)
            {
                UnitSlice s = sdao.retrieve(u.Id_concrete, u.Id_unit, u.Id_index, id_inner, id_index_inner);
                if (s != null)
                {
                    string id_unit_slice = s.Id_unit_slice;
                    int id_index = s.Id_index;

                    int rank_slice = rankOf(id_concrete, id_inner, id_index_inner, id_unit_slice, id_index);

                    rankMapping.Add(rank_slice, rank_unit);
                }
                rank_unit++;
            }

            int[] innerRanks = new int[rankMapping.Count];
            foreach (KeyValuePair<int, int> kvp in rankMapping)
            {
                int rank_slice = kvp.Key;
                rank_unit = kvp.Key;
                innerRanks[rank_slice] = ranks[rank_unit];
            }

            return innerRanks;
        }

        private int rankOf(int id_concrete, string id_inner, int id_index_inner, string id_unit_slice, int id_index)
        {
