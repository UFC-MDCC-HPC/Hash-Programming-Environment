...
*/
			                      ...
			                      ref IList<string> platform_list)
		{
			...
			foreach (SAFeSWL_BindingService e in env_bindings)
			{
				...
				{
					string cRef = e.provider_port.id_component; //prov_port_ownership [e.provider_port.id_port];
					role = platform_list.Contains(cRef) ? EnvironmentPortType.platform_user : EnvironmentPortType.user;
				...
				{					
                    ...
                    string cRef = e.user_port.id_component; //uses_port_ownership [e.user_port.id_port];
				...
				else  // binding entre dois componentes de solução ... 
				{
                    ...
                    string cRefUser = e.user_port.id_component;  //uses_port_ownership[e.user_port.id_port];
					string pRefUser = e.user_port.id_port;
					int facetUser = e.user_port.facet;
                    string cRefProvider = e.provider_port.id_component;   //prov_port_ownership[e.provider_port.id_port];
			...
			foreach (SAFeSWL_BindingAction bt in workflow_architecture.action_binding) 
			{
				...
				foreach (SAFeSWL_Port pt in bt.peer) 
				{
                    string cRef = pt.id_component;
				...
				foreach (Tuple<string,string,int> peer in binding.Item2)
				{
					...
					if (!cRef.Equals (proxy_name)) 
					{
						...
						if (!w.ContainsKey (cRef + pRef)) 
						{
                            ...
                            if (ic == null) 
                                Console.WriteLine("ERROR: pRef = {0}", pRef);
