            ...
            public static void determineArguments(IDictionary<string, int> arguments, int id_functor_app, out IDictionary<string, int> arguments_new)
            {
                ...
                foreach (SupplyParameter sp in spcList)
                {
                    ...
                    else if (sp is SupplyParameterComponent)
                    {
						...
						Trace.WriteLine("adding parameter (determineArguments) 2 key=" + spc.Id_parameter + "; value="+ spc.Id_functor_app_actual + " / " + rrrr.Id_abstract);
...
//				}
		...
		public static AbstractComponentFunctorApplication loadACFAFromInstantiator(Instantiator.ComponentFunctorApplicationType instantiator)
				                                     ...
				                                     gov.cca.Services frwServices)
			{
				...
				Trace.WriteLine("onInitializeApplication - START - Connecting to the Initialize port - " + cid.InstanceName);
				...
				Trace.WriteLine("onInitializeApplication - END - Connecting to the Initialize port - " + cid.InstanceName);
				                                     ...
				                                     gov.cca.Services frwServices)
			{
				...
				Trace.WriteLine("afterInitializeApplication - START - Connecting to the Initialize port - " + cid.InstanceName);
				frwServices.registerUsesPort(Constants.INITIALIZE_PORT_NAME, Constants.INITIALIZE_PORT_TYPE, new TypeMapImpl());
				ConnectionID conn_cs = bsPort.connect(frwServices.getComponentID(), Constants.INITIALIZE_PORT_NAME, cid, Constants.INITIALIZE_PORT_NAME);            
				InitializePort initialize_port = (InitializePort) frwServices.getPort (Constants.INITIALIZE_PORT_NAME);
				Trace.WriteLine("afterInitializeApplication - END - Connecting to the Initialize port - "+ cid.InstanceName);
			...
			{	
	                ...
	                {						
						if (!inner_cids.ContainsKey(portName)) 
						{
							...
							{  
							    ...
							    {
									Trace.WriteLine("IGNORING INNER COMPONENT " + cid.InstanceName  + "." + portName);
								}
							}
				...
				foreach (KeyValuePair<InnerComponent,Instantiator.UnitMappingType[]> ic_ in icList2)
				{
					...
					if (inner_cids.TryGetValue(portName, out inner_cid)) 
					{
						...
						Trace.Write ("createInnerComponentsOf:");
						Trace.WriteLine (" id_abstrac_owner=" + ic.Id_abstract_owner + ", id_inner=" + ic.Id_inner);
			...
			{	
				...
				foreach (InnerComponent ic in icList) 
				{
					...
					if (inner_facet >= 0) 
					{
							...
							//Tuple<int, Tuple<int,string>[]> facet_info = (Tuple<int, Tuple<int,string>[]>) facet_info_;
					...
					{
						Trace.WriteLine ("IGNORING inner component " + ic.Id_inner);
					}
				} 					
				...
				foreach (Slice slice in slice_list)
				{
					...
					if (unit_mapping_dict.TryGetValue (i_host.Id_interface_super_top, out info_list)) 
					{
						foreach (Tuple<int,int[],int> info in info_list) 
						{
							...
							Interface i = BackEnd.idao.retrieve (ic.Id_abstract_inner, slice.Id_interface_slice);
							...
							if (unit_mapping_save.TryGetValue (i.Id_interface_super_top + i_host.Facet, out unit_mapping_slice)) 
							{
							...
							else 
							{
								...
								unit_mapping_save.Add (unit_mapping_slice.unit_id + i_host.Facet, unit_mapping_slice);
