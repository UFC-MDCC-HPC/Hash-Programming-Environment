	...
	where C:IClass {
		...
		public override void compute() {
            ...
            for(k = 2; k<= nz - 1; k++) {
                for(j = jst; j<= jend; j++) {
                    for(i = L1; i<= L2; i++) {
                        flux[k-1, j, i, 0] = rsd[k-1, j+1, i+1, 1];
                        u21           = rsd[k-1, j+1, i+1, 1]/rsd[k-1, j+1, i+1, 0];
                        ...
                        flux[k-1, j, i, 1] =     rsd[k-1, j+1, i+1, 1]*u21 + c2*(rsd[k-1, j+1, i+1, 4] - q);
                        flux[k-1, j, i, 2] =     rsd[k-1, j+1, i+1, 2] * u21;
                        flux[k-1, j, i, 3] =     rsd[k-1, j+1, i+1, 3] * u21;
                        flux[k-1, j, i, 4] = (c1*rsd[k-1, j+1, i+1, 4] - c2*q)*u21;
                    }
                }
            }
            for(k = 2; k<= nz - 1; k++) {
            ...
            for(k = 1; k<= nz; k++) {
                for(j = jst; j<= jend; j++) {
                    for(i = ist; i<= iend; i++) {
                        flux[k-1, j, i, 0] = rsd[k-1, j+1, i+1, 3];
                        u41 = rsd[k-1, j+1, i+1, 3] / rsd[k-1, j+1, i+1, 0];
                        q = 0.50d*(rsd[k-1, j+1, i+1, 1]*rsd[k-1, j+1, i+1, 1]+rsd[k-1, j+1, i+1, 2]*rsd[k-1, j+1, i+1, 2]+rsd[k-1, j+1, i+1, 3]*rsd[k-1, j+1, i+1, 3])/rsd[k-1, j+1, i+1, 0];
