...
!
    ...
    do
      ...
      x(1:dim_num) = &
       ( real ( 2 * sub_num2(1:dim_num) - 2 * iwork(1:dim_num) + 1, kind = 8 ) * a(1:dim_num)   &
       + real (                         + 2 * iwork(1:dim_num) - 1, kind = 8 ) * b(1:dim_num) ) &
...
!
!    Input, integer ( kind = 4 ) DIM_NUM, the spatial dimension.  1 <= DIM_NUM <= 10.
!
