...
public class HCLParser implements HCLParserConstants {
 ...
 Component component = new Component();boolean flag=false;Import imp;Imports_list import_list = new Imports_list();Token kind_tk;Public_Component_list public_component_list;Inner_Component inner_comp=null;Inner_Component_list inner_comp_list = new Inner_Component_list();Unit_list unit_list = new Unit_list();Unit unit;
    ...
    while (true) {
      ...
      imp = imports(flag);
                             flag=true;import_list.add(imp);
    }
                                                                  component.setImports_list(import_list);
    kind_tk = kind();
  ...
  final public Import imports(boolean flag) throws ParseException {
 Token import_tk;Token id_tk;String nPackage;
    import_tk = jj_consume_token(IMPORT);
    nPackage = namePackage();
                                                   if ( (flag) && ((import_tk.specialToken==null) || (import_tk.specialToken.kind != NEWLINE))) { {if (true) throw new ParseException("New line expected before uses statement in line "+import_tk.beginLine+" and column "+import_tk.beginColumn);}}
                                                                                                                                                                                                                                                                                             {if (true) return new Import(nPackage,import_tk.beginLine,import_tk.beginColumn);}
    throw new Error("Missing return statement in function");
  }

  final public String namePackage() throws ParseException {
 Token id_tk;String pack="";
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case SLICE:
      id_tk = jj_consume_token(SLICE);
                       pack+=id_tk.image;
      label_4:
      while (true) {
        ...
        case ID:
          ...
          break;
        case FROM:
          id_tk = jj_consume_token(FROM);
          break;
        case UNIT:
          id_tk = jj_consume_token(UNIT);
          break;
        case IMPORT:
          id_tk = jj_consume_token(IMPORT);
          break;
        case COMPUTATION:
          id_tk = jj_consume_token(COMPUTATION);
          break;
        case DATA:
          id_tk = jj_consume_token(DATA);
          break;
        case ARCHITECTURE:
          id_tk = jj_consume_token(ARCHITECTURE);
          break;
        case ENVIRONMENT:
          id_tk = jj_consume_token(ENVIRONMENT);
          break;
        case QUALIFIER:
          id_tk = jj_consume_token(QUALIFIER);
          break;
        case APPLICATION:
          id_tk = jj_consume_token(APPLICATION);
          break;
        case ENUMERATOR:
          id_tk = jj_consume_token(ENUMERATOR);
          break;
        case SYNCHRONIZER:
          id_tk = jj_consume_token(SYNCHRONIZER);
          break;
        case SLICE:
          id_tk = jj_consume_token(SLICE);
          break;
 ...
 Token id_tk;Token kind_tk;Type type;Public_Component_list public_inner_component_list = new Public_Component_list();
    kind_tk = kind();
    id_tk = jj_consume_token(ID);
                                   if ((kind_tk.specialToken==null) || (kind_tk.specialToken.kind != NEWLINE)) { {if (true) throw new ParseException("New line expected before kind of inner component in line "+kind_tk.beginLine+" and column "+kind_tk.beginColumn);}}
    jj_consume_token(COLON);
    type = cFunApp();
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case OCURV:
 ...
 Token extend_tk;Config config;
    ...
    case EXTENDS:
      ...
      config = cFunAppNoVar();
                                                   {if (true) return new Extend(config,extend_tk.beginLine,extend_tk.beginColumn,1);}
      break;
    case IMPLEMENTS:
      extend_tk = jj_consume_token(IMPLEMENTS);
      config = cFunAppNoVar();
                                                        {if (true) return new Extend(config,extend_tk.beginLine,extend_tk.beginColumn,2);}
      break;
    default:
      jj_la1[38] = jj_gen;
      jj_consume_token(-1);
 ...
 Public_Component public_component;
  ...
  static {
   ...
   private static void jj_la1_init_0() {
      jj_la1_0 = new int[] {0x4000,0x7f8000,0x2000,0xfff800,0x20,0xfff800,0x20,0xfff800,0x20,0xfff800,0x20,0xfff800,0x20,0xfff800,0x20,0xfff800,0x20,0xfff800,0x20,0xfff800,0x20,0xfff800,0x20,0xfff800,0x20,0xfff800,0x20,0xfff800,0x20,0xfff800,0x80,0x200,0x6,0x800000,0x200,0x80,0x800,0x7f8000,0x6,0x10,0x10,0x10,};
   }
