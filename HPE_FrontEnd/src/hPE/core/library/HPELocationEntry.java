...
public class HPELocationEntry {
	...
	public static List<String> fetchPackagesFromLocation(URI locationSite, boolean showObsolete) throws RemoteException, ServiceException {
		try {
			...
			/**
			 * o valor dessa string retornada pelo webservice ï¿½ algo do seguinte formato:
			 * package1 ( component1 component2 ... ) package2 ( ) package3 ( component1 ) ...
			 * cada package ï¿½ seguida pelo conjunto de components que possui. 
			 * Se houver algum erro, o valor da str serï¿½ null
			 */
			if(str!=null){
				...
				while(token.hasMoreTokens()){
					...
					while(token.hasMoreTokens() && !aux.equals(")")){						
						c += aux+" "; //vai armazenar os components de um package separados por espaï¿½o
						aux = token.nextToken(); // aux recebe ) ou Component					
					}			
					packagesList.add(p);			
	...
	public static List<String> fetchComponents(URI locationSite, String[] packagePath) {
		...
		if(!htable.containsKey(pk)){ //se a package ainda nï¿½o estï¿½ na tabelahash devemos fzr um fetchpackages para adiciona-la
			try {
