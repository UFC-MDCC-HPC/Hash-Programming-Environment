...
public class LPackage extends HPEComponentLibraryItem implements ILPackage {
	...
	public LPackage(IHPEComponentLibrary parentLibrary, String[] packagePath) {
		...
		title = putSeparator(getPackagePath(), ".", false);
	...
	protected void addComponent(URI locationSite, List<String> components) {
		...
		for (String componentNameAndVersion : components) {
			...
			String componentName = "<unknown> (error)";
			String version = null;
	 ...
	 * { if (isAllowedToModifyPackage()) { ILComponentView componentView = new
	 * LComponentView(this,componentConfig.getHeader().getName(),locationSite);
	 * this.registerNewComponent(componentConfig); this.addChild(componentView);
	 * } }
	 */

	/*
	 * public void removeComponent(ILComponentView theComponent) { if
	 * (isAllowedToModifyPackage()) {
	 * 
	 * // TODO THE line below ... //
	 * this.unregisterComponent(theComponent.getComponent(false));
	 * this.removeChild(theComponent); } else { // ERROR: operation now allowed
	 * (remove package). } }
	 */
